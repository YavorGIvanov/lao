...
1. Detects whichever of Codex and Claude Code the user has, existing local endpoints, Git, and supported hardware; only selected clients are managed.
...
Current packaging preparation: `package.sh` produces an unsigned Apple Silicon archive whose installer needs no source tooling on the receiving machine. It checks a fixed checksum manifest before installing binaries. This is a contributor test path, not authenticated distribution or a completed one-command release. The archive includes dependency and model notices in its fixed checksum manifest; downloaded models and runtimes are identified separately from bundled code. Binary replacement stages the pair and retains originals in a checksummed recovery record. A kernel lock releases after process death; the next installer restores interrupted work before proceeding, while conflicts or invalid snapshots stop recovery. `lao uninstall` reuses the settings restoration/shutdown path, then deletes LAO-owned application state, downloaded caches and binaries whose ownership checksums still match. It preserves external runtimes and client homes; conflicts retain recovery material. `off` continues to retain cached artifacts and installed binaries. Signing, notarization, publication, physical power-loss testing and full service/settings upgrade evidence remain R11 requirements.
...
There is no replacement REPL or mandatory delegation. The original client remains responsible for its terminal UI, planning, approvals, context, and verification. For a bounded implementation packet it may call the optional `lao.execute` MCP tool; LAO then routes that packet to Cloud or to a permission-bounded OpenCode worker. The tested Codex integration installs one short developer instruction for this handoff and refuses to overwrite an existing value. The tested Claude Code version uses the same explicit MCP tool contract without another managed instruction file.
...
Installation automatically selects available harnesses or accepts `--client codex|claude|both`. Settings, recovery snapshots, local caller capabilities and warming probes belong only to selected clients. Existing two-client records remain restorable, and upgrades retain the recorded selection; changing it requires `off` first.

Version admission uses Codex 0.151.0 and Claude Code 2.1.251 as minimums, then checks the CLI flags required by LAO’s local probes. Newer versions can pass without an allowlist update. Missing capabilities, older versions and unrecognized version strings fail preflight. Local warming and smoke exercise the actual installed client; neither help output nor a local canary certifies native-cloud compatibility. Record exact tested versions in the release evidence rather than turning the evidence list into an installation whitelist. Codex’s public [CLI reference](https://developers.openai.com/codex/cli/reference/) documents the strict configuration, ephemeral, sandbox and non-interactive flags used here.

### 7.1 Codex

Codex supports custom providers and local providers through its configuration. The narrow compatibility path uses a managed custom provider named `lao`, a non-secret loopback `/oai` prefix, `requires_openai_auth = true`, `supports_websockets = false`, and a separate `X-LAO-Key` caller header. This lets Codex reuse its current native authentication without LAO reading auth storage, while the private caller token authenticates payload requests to the local gateway. Codex 0.151.0 has proven this HTTP/SSE shape first with a synthetic credential and then with its saved ChatGPT login through the native gate. Treat this as evidence for that version, not a permanent provider guarantee. The user-level setting is also visible to the Codex IDE extension. See the [Codex configuration reference](https://developers.openai.com/codex/config-reference/) and [Codex hooks](https://developers.openai.com/codex/hooks/).
...
The caller token does not authenticate the gateway to the client. Activation therefore requires the OS supervisor to own the loopback socket before any client config is changed and to retain that socket across daemon crashes and restarts. On the Apple-first PoC this is a per-user launchd socket. Install requires both a fresh 0600 adoption file and the exact local hello before changing client configuration; `launchctl bootstrap` success alone is insufficient when a port is already occupied. Rollback finishes before the supervisor releases the listener. The LaunchAgent guarantee lasts only while its GUI login domain is loaded. Boot-wide persistence still needs a privileged system LaunchDaemon or rollback/re-activation around logout and login. Hostile pre-bind, daemon-crash, delayed-start, and rollback proofs cover the PoC boundary.
...
The allowlist rejects absolute paths, Git metadata, symlinks, hardlinked or special files, wildcard characters (`*`, `?`), and backslashes before execution. Repository roots must also be literal permission paths. Pinned OpenCode enforces exact read/edit tool permissions. R7 adds a separate macOS sandbox around the worker and its children: repository contents are limited to the allowlist, private writes to disposable state, and TCP connections to the local gate port. The process can inspect filesystem metadata and top-level repository names, read its verified support tree and standard system resources, and use Apple's baseline system IPC. It cannot read Git metadata or unlisted repository contents. Paths enter the sandbox profile as parameters, never executable profile text. With Git hidden, OpenCode's tool permission paths are expressed relative to `/`.

The sandbox uses deprecated `/usr/bin/sandbox-exec` and Apple's private `system.sb` baseline. Unsupported platforms and sandbox startup failures fail closed; there is no unrestricted fallback. This is a supported-Mac proof, not a portable sandbox or protection against another unsandboxed process under the same user racing filesystem changes. New files can be written only within existing directories; the worker gets no general directory-creation grant in the repository.

Completion requires successful process exit, bounded valid event output ending with a terminal stop, no reported tool/session failure, and an observed allowed-file change. Changed paths are detected among allowed files, not by a whole-repository audit. The parent harness reviews the actual diff and verifies the outcome, including partial edits after failure. Execution status alone is not quality certification.
