...
This code remains private and unwired from the daemon. Hyper owns HTTP/1 framing; policy does not poll the application body. Test code may map the already-frozen semantic target to a loopback fixture. Native connections resolve once under a deadline, reject the full result if any address is non-public, connect the resolved address directly, and complete platform certificate and hostname verification before Hyper can send native authentication. Private-address enterprise interception therefore fails closed in the PoC. Direct HTTP accepts only the fixed local loopback target. The exact Claude hello now returns an empty local response. Supervisor activation, sealed router handoff, full errors/retries/cancellation, and configuration remain unimplemented.
