...
P0-00 and P0-01 are complete. The private P0-04 gate now hands only client and operation to a separate router, which defaults to cloud; request bodies, headers, credentials, and native targets remain private to the gate. The path uses bounded DNS, fixed semantic targets, platform-verified TLS, and Hyper HTTP/1. Codex 0.146.0 passed another cheap saved-login E2E through this seam. Claude's prior 2.1.223 E2E remains valid, but the current harness is logged out; 2.1.227+ still needs a fresh probe. Local production routing, logout/reboot continuity, and real configuration writes remain off.
