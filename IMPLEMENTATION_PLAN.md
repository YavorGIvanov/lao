...
P0-04 remains open. The code is private and unwired from the daemon. Test code can map an already-frozen semantic target to a loopback fixture. Native TLS and bounded DNS are implemented, direct HTTP accepts only the fixed local loopback target, and the exact Claude hello has an empty local response. Supervisor-held socket activation, sealed router handoff, complete errors/retries/cancellation, and daemon wiring remain unimplemented. Hyper 1.11.0, hyper-util 0.1.20, Tokio 1.53.1, rustls 0.23.43, tokio-rustls 0.26.4, and rustls-platform-verifier 0.7.0 are narrowly pinned.
